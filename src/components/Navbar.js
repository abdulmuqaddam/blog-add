            ...
            {loadingCategories ? (
              ...
              navLinks.map((link) => {
                ...
                return (
                  ...
                  >
                    {link.name}
                    <span className={`absolute left-4 right-4 h-0.5 bg-indigo-600 transform scale-x-0 group-hover:scale-x-100 transition-transform duration-300 ease-out bottom-0 ${isActive ? 'scale-x-100' : ''}`} />
                  </Link>
                );
              })
            )}
